...
import type fetchCourse from "@/api/course";
import { PropsWithLng } from "@/app/i18next";
import { useTranslation } from "@/app/i18next/client";

type Course = Awaited<ReturnType<typeof fetchCourse>>[number];

const columnHelper =
  createColumnHelper<Awaited<ReturnType<typeof fetchCourse>>[number]>();

const Inner = ({ courses, lng }: PropsWithLng<{ courses: Course[] }>) => {
  const { t } = useTranslation(lng);
  const columns = useMemo(
    () => [
      columnHelper.accessor("code", { header: t("code") }),
